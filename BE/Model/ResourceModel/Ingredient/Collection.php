...
/**
 * Class Collection
 * @package Custom\Ingredients\Model\ResourceModel\Ingredient
...
class Collection extends AbstractCollection
{
    ...
    {
        $this->_init('Custom\Ingredients\Model\Ingredient', 'Custom\Ingredients\Model\ResourceModel\Ingredient');
    }
    ...
    public function addCategoriesFilter(array $categoriesFilter)
    {
        foreach ($categoriesFilter as $conditionType => $values) {
            $categorySelect = $this->getConnection()->select()->from(
                ['cat' => $this->getTable('custom_ingredient_ingredients_category')],
                'cat.ingredient_id'
    ...
    {
        $this->getSelect()
            ->where("EXISTS (SELECT * FROM `{$this->getTable('custom_ingredient_product')}`
                AS `ingredient_product`
                WHERE e.entity_id = ingredient_product.ingredient_id
                AND ingredient_product.product_id in (?))", [0, $product->getId()]);
        return $this;
    }
}
